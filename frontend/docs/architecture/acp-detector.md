...
`@zed-industries/claude-agent-acp` 是由 Zed 维护的 ACP bridge，它在内部负责启动和管理 Claude Code CLI。SalomoneUI 与这个 bridge 通过 stdin/stdout 上的 JSON-RPC (NDJSON) 进行通信。
...
| `AcpAdapter`            | `AcpAdapter.ts`            | ACP session update → SalomoneUI TMessage 格式转换     |
...
> 上文分析了 SalomoneUI 侧如何通过 npx 启动 `@zed-industries/claude-agent-acp`。
...
适配器通过 **stdin/stdout NDJSON** 与上游 ACP client（SalomoneUI / Zed 编辑器等）通信。`AgentSideConnection` 负责 JSON-RPC 分发。
...
## 与上文 SalomoneUI 侧的连接关系

SalomoneUI 通过 npx 启动 `claude-agent-acp`（上文 Phase 3），bridge 进程启动后：

1. SalomoneUI 与 bridge 之间 = **ACP 协议**（上文 Phase 4-8）
