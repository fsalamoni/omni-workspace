# SalomoneUI Headless Server Deployment Guide

Deploy SalomoneUI WebUI on headless Linux servers — cloud VMs, Kubernetes Pods, and containers — with proxy auto-fallback support.
...
- At least 2GB RAM
- SalomoneUI `.deb` package from [Releases](https://github.com/iOfficeAI/SalomoneUI/releases)
...
wget https://github.com/iOfficeAI/SalomoneUI/releases/latest/download/SalomoneUI-linux-amd64.deb

# Install
sudo dpkg -i SalomoneUI-linux-amd64.deb
...
SalomoneUI is an Electron app and requires a display server. On headless servers (no monitor), use Xvfb to create a virtual display:
...
Create `/opt/SalomoneUI/start-salomoneui.sh`:

```bash
#!/bin/bash
# SalomoneUI WebUI headless startup script
# Usage: ./start-salomoneui.sh [start|stop|restart|status]

PIDFILE="/var/run/salomoneui.pid"
LOGFILE="/var/log/salomoneui.log"
WORKDIR="$HOME"  # Change to your workspace directory

start() {
    if [ -f "$PIDFILE" ] && kill -0 "$(cat $PIDFILE)" 2>/dev/null; then
        echo "SalomoneUI is already running (PID: $(cat $PIDFILE))"
        return 1
    fi
    ...
    nohup xvfb-run --auto-servernum --server-args="-screen 0 1920x1080x24" \
        /usr/bin/SalomoneUI --webui --remote --no-sandbox \
        > "$LOGFILE" 2>&1 &
    echo $! > "$PIDFILE"
    sleep 3
    if kill -0 "$(cat $PIDFILE)" 2>/dev/null; then
        echo "SalomoneUI started successfully (PID: $(cat $PIDFILE))"
        echo "WebUI: http://$(hostname -I | awk '{print $1}'):25808"
    else
        echo "SalomoneUI failed to start. Check log: $LOGFILE"
        rm -f "$PIDFILE"
        return 1
    fi
}
...
stop() {
    if [ ! -f "$PIDFILE" ]; then
        echo "SalomoneUI is not running (no PID file)"
        return 1
    fi
    PID=$(cat "$PIDFILE")
    echo "Stopping SalomoneUI (PID: $PID)..."
    kill "$PID" 2>/dev/null
    sleep 2
    kill -9 "$PID" 2>/dev/null
    pkill -f "SalomoneUI --webui" 2>/dev/null
    rm -f "$PIDFILE"
    echo "SalomoneUI stopped."
}
...
status() {
    if [ -f "$PIDFILE" ] && kill -0 "$(cat $PIDFILE)" 2>/dev/null; then
        echo "SalomoneUI is running (PID: $(cat $PIDFILE))"
        ss -tlnp | grep 25808
    else
        echo "SalomoneUI is not running."
...
```bash
chmod +x /opt/SalomoneUI/start-salomoneui.sh
```

> **Tip**: `WORKDIR` determines the directory SalomoneUI can access for file operations. Set it to your project workspace.

---

## Remote Access

SalomoneUI WebUI listens on port **25808**. Choose a method based on your network setup:
...
### Step 2: PAC File for SalomoneUI (Electron / Chromium Layer)
...
Create `/opt/SalomoneUI/proxy.pac`:
...
```bash
    nohup xvfb-run --auto-servernum --server-args="-screen 0 1920x1080x24" \
        /usr/bin/SalomoneUI --webui --remote --no-sandbox \
        --proxy-pac-url="file:///opt/SalomoneUI/proxy.pac" \
        > "$LOGFILE" 2>&1 &
```
...
### Step 4: SalomoneUI Internal Proxy (Gemini API)

For Gemini API calls, configure the proxy inside SalomoneUI WebUI:

**Settings → Gemini Settings → Proxy** → `http://127.0.0.1:7897`

> This proxy is handled by SalomoneUI's Node.js layer (separate from the Chromium layer). When the SSH tunnel is down, Gemini API calls will fail, but the WebUI and other APIs remain functional.
...
| Issue                                     | Solution                                                     |
| ----------------------------------------- | ------------------------------------------------------------ |
| `dpkg` dependency errors in containers    | `dpkg --force-all -i SalomoneUI-linux-amd64.deb`                 |
| SalomoneUI can only access `/tmp`             | Set `WORKDIR` in the startup script to your workspace path   |
...
# SalomoneUI 无头服务器部署指南

在无图形界面的 Linux 服务器（云主机、K8s Pod、容器）上部署 SalomoneUI WebUI，支持代理自动回退。

## 前置条件

- Linux x86_64（推荐 Ubuntu 20.04+ / Debian 11+）
- 至少 2GB 内存
- SalomoneUI `.deb` 安装包（[下载地址](https://github.com/iOfficeAI/SalomoneUI/releases)）
...
SalomoneUI 是 Electron 应用，需要显示服务。无头服务器需安装 Xvfb：
...
创建 `/opt/SalomoneUI/start-salomoneui.sh`：

```bash
#!/bin/bash
# SalomoneUI WebUI 无头启动脚本
# 用法: ./start-salomoneui.sh [start|stop|restart|status]
...
start() {
    if [ -f "$PIDFILE" ] && kill -0 "$(cat $PIDFILE)" 2>/dev/null; then
        echo "SalomoneUI 已在运行 (PID: $(cat $PIDFILE))"
        return 1
    fi
    echo "正在启动 SalomoneUI WebUI..."
    cd "$WORKDIR"
    ...
    sleep 3
    if kill -0 "$(cat $PIDFILE)" 2>/dev/null; then
        echo "SalomoneUI 启动成功 (PID: $(cat $PIDFILE))"
        echo "WebUI: http://$(hostname -I | awk '{print $1}'):25808"
    else
        echo "SalomoneUI 启动失败，请查看日志: $LOGFILE"
        rm -f "$PIDFILE"
        return 1
    fi
}
...
stop() {
    if [ ! -f "$PIDFILE" ]; then
        echo "SalomoneUI 未在运行"
        return 1
    fi
    PID=$(cat "$PIDFILE")
    echo "正在停止 SalomoneUI (PID: $PID)..."
    kill "$PID" 2>/dev/null
    sleep 2
    kill -9 "$PID" 2>/dev/null
    pkill -f "SalomoneUI --webui" 2>/dev/null
...
status() {
    if [ -f "$PIDFILE" ] && kill -0 "$(cat $PIDFILE)" 2>/dev/null; then
        echo "SalomoneUI 运行中 (PID: $(cat $PIDFILE))"
        ss -tlnp | grep 25808
    else
        echo "SalomoneUI 未在运行。"
        rm -f "$PIDFILE" 2>/dev/null
...
SalomoneUI WebUI 监听端口 **25808**，根据网络环境选择访问方式：
...
### 第二步：PAC 代理文件（SalomoneUI Electron 层）

`--proxy-server` 的问题：代理一断，**所有请求**全挂。改用 PAC 文件实现自动回退。

创建 `/opt/SalomoneUI/proxy.pac`：
...
启动脚本中添加参数：`--proxy-pac-url="file:///opt/SalomoneUI/proxy.pac"`
...
### 第四步：SalomoneUI 内置代理（Gemini API）
...
| SalomoneUI 只能访问 /tmp   | 修改启动脚本中的 `WORKDIR`            |
