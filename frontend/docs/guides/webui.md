# SalomoneUI WebUI Mode - Startup Guide

SalomoneUI supports WebUI mode, allowing you to access the application through a web browser. This guide covers how to start WebUI mode on all supported platforms.
...
WebUI mode starts SalomoneUI with an embedded web server, allowing you to:

- Access the application through any modern web browser
- Use SalomoneUI from remote devices on the same network (with `--remote` flag)
...
```cmd
# Using full path
"C:\Program Files\SalomoneUI\SalomoneUI.exe" --webui

# Or if SalomoneUI is in your PATH
SalomoneUI.exe --webui
```
...
```bash
# Using full path
/Applications/SalomoneUI.app/Contents/MacOS/SalomoneUI --webui

# Or using open command
open -a SalomoneUI --args --webui
```

### Method 2: Create Shell Script

Create `start-salomoneui-webui.sh`:
...
```bash
chmod +x start-salomoneui-webui.sh
./start-salomoneui-webui.sh
```
...
# Or using full path
/opt/SalomoneUI/salomoneui --webui
```
...
chmod +x SalomoneUI-*.AppImage

# Run with --webui flag
./SalomoneUI-*.AppImage --webui
```

### Method 2: Create Desktop Entry

Create `~/.local/share/applications/salomoneui-webui.desktop`:

```ini
[Desktop Entry]
Name=SalomoneUI WebUI
Comment=Start SalomoneUI in WebUI mode
Exec=/opt/SalomoneUI/salomoneui --webui
Icon=salomoneui
Terminal=false
Type=Application
...
```bash
chmod +x ~/.local/share/applications/salomoneui-webui.desktop
```
...
Create `~/bin/start-salomoneui-webui.sh`:
...
```bash
chmod +x ~/bin/start-salomoneui-webui.sh
```
...
Create `/etc/systemd/system/salomoneui-webui.service`:

```ini
[Unit]
Description=SalomoneUI WebUI Service
After=network.target

[Service]
Type=simple
User=YOUR_USERNAME
ExecStart=/opt/SalomoneUI/salomoneui --webui --remote
Restart=on-failure
RestartSec=10
...
sudo systemctl enable salomoneui-webui.service
sudo systemctl start salomoneui-webui.service

# Check status
sudo systemctl status salomoneui-webui.service
```
...
**Important Note**: Electron desktop mode is **not supported** on Android. However, you can run SalomoneUI in WebUI mode using Termux with a prooted Linux environment.

> **Community Contribution**: This guide is contributed by [@Manamama](https://github.com/Manamama). Special thanks for making SalomoneUI accessible on Android devices! 🙏
>
> **Original Tutorial**: [Running SalomoneUI WebUI on Android via Termux + Proot Ubuntu](https://gist.github.com/Manamama/b4f903c279b5e73bdad4c2c0a58d5ddd)
>
> **Related Issues**: [#217 - Android Support Discussion](https://github.com/iOfficeAI/SalomoneUI/issues/217)
...
#### 4. Download and Install SalomoneUI

```bash
# Download the ARM64 .deb package (replace VERSION with the actual version)
# Check latest version at: https://github.com/iOfficeAI/SalomoneUI/releases
wget https://github.com/iOfficeAI/SalomoneUI/releases/download/vVERSION/SalomoneUI_VERSION_arm64.deb

# Example (replace VERSION with the release tag, e.g. v1.5.2):
wget https://github.com/iOfficeAI/SalomoneUI/releases/download/vVERSION/SalomoneUI_VERSION_arm64.deb

# Install the package
apt install -y ./SalomoneUI_*.deb

# Verify installation
which SalomoneUI
```

#### 5. Launch SalomoneUI WebUI

```bash
# Start SalomoneUI in WebUI mode with no-sandbox flag
SalomoneUI --no-sandbox --webui
```
...
To access SalomoneUI from other devices on your local network:

```bash
# Start with --remote flag
SalomoneUI --no-sandbox --webui --remote
...
SalomoneUI --no-sandbox --webui --port 8080
```
...
chmod +x /opt/SalomoneUI/salomoneui
```

#### Out of Memory

SalomoneUI requires sufficient RAM. Close other apps if you encounter memory issues.

#### Cannot Access from Browser

1. Check if SalomoneUI is running: look for "Server started" message
...
4. **Keep device charged** - Running SalomoneUI consumes battery

### Tested Environment

- **Device**: Android 14
- **Termux Version**: 0.118.0
- **SalomoneUI Version**: Latest release (e.g. 1.5.2)
- **Proot-distro**: Ubuntu (latest)

### Creating a Startup Script

For convenience, create a script to launch SalomoneUI quickly:

```bash
# Create script in Ubuntu (proot)
cat > ~/start-salomoneui.sh << 'EOF'
#!/bin/bash
echo "Starting SalomoneUI WebUI..."
SalomoneUI --no-sandbox --webui --remote
EOF

# Make executable
chmod +x ~/start-salomoneui.sh
...
```bash
proot-distro login ubuntu -- bash -c "SalomoneUI --no-sandbox --webui --remote"
```
...
2. Report issues at [GitHub Issues #217](https://github.com/iOfficeAI/SalomoneUI/issues/217)
...
```cmd
SalomoneUI.exe --webui --remote
```

### macOS

```bash
/Applications/SalomoneUI.app/Contents/MacOS/SalomoneUI --webui --remote
```

### Linux

```bash
salomoneui --webui --remote
```
...
netsh advfirewall firewall add rule name="SalomoneUI WebUI" dir=in action=allow protocol=TCP localport=3000
```
...
**macOS:**
Go to **System Preferences** → **Security & Privacy** → **Firewall** → **Firewall Options** → Add SalomoneUI
...
```cmd
where SalomoneUI.exe
```

**macOS:**

```bash
mdfind -name "SalomoneUI.app"
```
...
which salomoneui
# or
find /opt -name "salomoneui" 2>/dev/null
```
...
```powershell
& "C:\Program Files\SalomoneUI\SalomoneUI.exe" --webui 2>&1 | Tee-Object -FilePath salomoneui.log
```

**macOS/Linux:**

```bash
/path/to/salomoneui --webui 2>&1 | tee salomoneui.log
```
...
| Platform | Location                                                 |
| -------- | -------------------------------------------------------- |
| Windows  | `%APPDATA%/SalomoneUI/webui.config.json`                     |
| macOS    | `~/Library/Application Support/SalomoneUI/webui.config.json` |
| Linux    | `~/.config/SalomoneUI/webui.config.json`                     |
...
```cmd
# Using full path
"C:\Program Files\SalomoneUI\SalomoneUI.exe" --resetpass

# Or for a specific user
"C:\Program Files\SalomoneUI\SalomoneUI.exe" --resetpass username
```
...
```bash
# Using full path
/Applications/SalomoneUI.app/Contents/MacOS/SalomoneUI --resetpass

# Or for a specific user
/Applications/SalomoneUI.app/Contents/MacOS/SalomoneUI --resetpass username
```
...
```bash
# Using system path
salomoneui --resetpass

# Or for a specific user
salomoneui --resetpass username

# Or using full path
/opt/SalomoneUI/salomoneui --resetpass
```
...
- [GitHub Issues](https://github.com/iOfficeAI/SalomoneUI/issues)
...
2. Search [existing issues](https://github.com/iOfficeAI/SalomoneUI/issues)
3. Create a [new issue](https://github.com/iOfficeAI/SalomoneUI/issues/new) with:
   - Your OS and version
   - SalomoneUI version
   - Steps to reproduce
...
**Happy using SalomoneUI in WebUI mode!** 🚀
