...
把 SalomoneUI 从「一个功能齐全的 AI 聊天客户端」变成 **「一个悬浮在桌面上、意图驱动的 AI 助手」**：
...
- ❌ 替换现有 SalomoneUI 主界面（通过 feature flag 隔离，默认关闭）
- ❌ 全套 i18n / a11y / 主题适配
- ❌ 生产级错误处理 / telemetry
...
1. 启动 SalomoneUI（ambient flag 开启）→ 看到气泡出现在右下
...
- **AC-M3-2**（P0）：消息流支持流式渲染（打字机效果），和当前 SalomoneUI 一致
...
- **AC-M5-4**（P1）：逃生阀切换行为 = **ambient 模式下展开全部面板**（等价于 `switch_scene('research')`），不回退到传统 SalomoneUI 主界面。demo 阶段不引入双主界面切换
...
- SalomoneUI 的 fork Gemini 是主线，Gemini 覆盖度最高
...
前缀 `ambient_` 避免与现有 builtin tool (`salomoneui_image_generation` 等) 冲突。
...
- 和现有 SalomoneUI 主模式的共存策略（设置里可切换默认形态）
