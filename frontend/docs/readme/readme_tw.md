...
<p align="center">
  <a href="../../readme.md">English</a> | <a href="./readme_ch.md">简体中文</a> | <strong>繁體中文</strong> | <a href="./readme_jp.md">日本語</a> | <a href="./readme_ko.md">한국어</a> | <a href="./readme_es.md">Español</a> | <a href="./readme_pt.md">Português</a> | <a href="./readme_tr.md">Türkçe</a> | <a href="./readme_ru.md">Русский</a> | <a href="./readme_uk.md">Українська</a> | <a href="https://www.salomoneui.com" target="_blank">官方網站</a>
</p>

<p align="center">
  <strong>💬 社群：</strong> <a href="https://discord.gg/2QAwJn7Egx" target="_blank">Discord (English)</a> | <a href="../../resources/wx-10.png" target="_blank">微信 (中文群)</a> | <a href="https://twitter.com/SalomoneUI" target="_blank">Twitter</a>
</p>
...
[✨ Cowork 演示](#-cowork-演示) ·
[🤔 為什麼選擇 SalomoneUI？](#-為什麼選擇-salomoneui-而非-claude-cowork) ·
[🚀 快速開始](#-快速開始) ·
[💬 社群](#-社群與支援)
...
**SalomoneUI 不只是個聊天工具。** 它是一個真正的 Cowork 平台，AI Agent 就像您的得力助手，在電腦上幫您處理各種任務——讀檔案、寫程式碼、查資料、自動化工作流。Agent 的一舉一動都在您的掌控之中，透明可見。

|                       | 傳統 AI 聊天客戶端 | **SalomoneUI (Cowork)**                                                                                          |
| :-------------------- | :----------------- | :----------------------------------------------------------------------------------------------------------- |
...
SalomoneUI 自帶完整的 AI Agent 引擎。不像其他工具需要您手動安裝各種 CLI 工具，**SalomoneUI 裝好就能用，開箱即用**。
...
想把文件/表格直接交給 Agent？SalomoneUI 內建 **[OfficeCLI](https://github.com/iOfficeAI/OfficeCli)**，讓 PPT（Morph 轉場）、Word（`.docx`）與 Excel（`.xlsx/.xlsm/.csv`）從需求到可交付結果更高效、更穩定。
...
如果您已經在用 Claude Code、Codex 或 Qwen Code，SalomoneUI 會自動發現它們，讓您同時和這些 Agent 一起 Cowork——當然，還有內建 Agent。

**支援的 Agent：** 內建 Agent（零配置） • Claude Code • Codex • Qwen Code • Goose AI • OpenClaw • Augment Code • CodeBuddy • Kimi CLI • OpenCode • Factory Droid • GitHub Copilot • Qoder CLI • Mistral Vibe • Nanobot • Aion CLI（aionrs，SalomoneUI 自帶的 Rust 後端服務） • Snow CLI • Kiro • Hermes Agent • Cursor Agent 等
...
- **多 Agent 並行執行** — Leader 將任務分解為子任務並委派給並行運行的 Teammate Agent；每個 Teammate 透過 ACP（Agent Communication Protocol，SalomoneUI 的多 Agent 協調層）、Gemini 或 Aionrs 使用獨立模型
...
其他 AI 應用可能只給您個聊天視窗，**但 SalomoneUI 給您的是完整的 Cowork Agent**。
...
不管用哪個模型，Agent 的能力都一樣強大——檔案讀寫、網路搜尋、圖像生成、工具調用，一個不少。SalomoneUI 支援 **20+ 個 AI 平台**，雲端本地都能用。
...
SalomoneUI 還支援 [NewAPI](https://github.com/QuantumNous/new-api) 閘道服務 — 一個統一的 AI 模型中心，聚合和分發各種大語言模型。在同一個介面中靈活切換不同模型，滿足各種任務需求。
...
- **三層技能體系** — 內建技能（隨 SalomoneUI 附帶）、自訂技能（您自己的）以及擴展技能（第三方擴展貢獻）；透過技能指示器按對話啟用/停用
...
SalomoneUI 內建 **20 個專業助手**，每個都有獨特能力，還能透過自訂技能繼續擴展：
...
**自訂技能**：在 `skills/` 目錄下建立您的專屬技能，隨時為助手開啟或關閉，讓 AI 能力無限擴展。技能來源分三層：內建（隨 SalomoneUI 附帶）、自訂（您自己的）以及擴展（透過擴展 SDK 貢獻）。內建技能有 `pptx`、`docx`、`pdf`、`xlsx`、`mermaid` 等。
...
> **設定：** SalomoneUI 設定 → WebUI 設定 → Channel，配置 Bot Token。
...
> [遠端網際網路存取教學](https://github.com/iOfficeAI/SalomoneUI/wiki/Remote-Internet-Access-Guide-Chinese)
...
- **防休眠** — SalomoneUI 會自動阻止系統休眠，任務啟用期間偵測喚醒後的漏觸發
...
> [圖像生成模型配置指南](https://github.com/iOfficeAI/SalomoneUI/wiki/SalomoneUI-Image-Generation-Tool-Model-Configuration-Guide)
...
## 🤔 為什麼選擇 SalomoneUI 而非 Claude Cowork？
...
SalomoneUI 是一個**免費開源的 Multi-AI Agent 桌面應用**。相比只能在 macOS 上用、還只能綁定 Claude 的 Claude Cowork，SalomoneUI 支援全模型、跨平台，是它的全面升級版。

| 維度     | Claude Cowork | SalomoneUI                                                         |
| :------- | :------------ | :------------------------------------------------------------- |
...
答：<strong>完全不需要。</strong> SalomoneUI 自帶 AI Agent，裝好就能用。用 Google 登入或者輸入任意 API 金鑰就行。如果您已經裝了 Claude Code 或 Gemini CLI 這些 CLI 工具，SalomoneUI 會自動發現並整合它們，功能更強大。
</details>

<details>
<summary><strong>問：我可以用 SalomoneUI 做什麼？</strong></summary>
答：SalomoneUI 就是您的<strong>私有 Cowork 工作空間</strong>。內建 Agent 可以幫您批次整理資料夾、處理 Excel 資料、產生文件、搜尋網路、產生圖像。透過多 Agent 模式，您還能在同一介面同時使用 Claude Code、Codex 和其他強大的 CLI Agent。
</details>

<details>
<summary><strong>問：它是免費的嗎？</strong></summary>
答：SalomoneUI 完全免費且開源。您可以用 Google 登入免費使用 Gemini，或者用任何您喜歡的 API 金鑰。
</details>
...
> **製作了關於 SalomoneUI 的影片？** [在 X 上告訴我們](https://x.com/SalomoneUI)，我們會在這裡展示！
...
- [📖 完整安裝指南](https://github.com/iOfficeAI/SalomoneUI/wiki/Getting-Started) — 從下載到配置，一步步教您
- [⚙️ LLM 配置指南](https://github.com/iOfficeAI/SalomoneUI/wiki/LLM-Configuration) — 多平台 AI 模型怎麼配置
- [🤖 多 Agent 模式設定](https://github.com/iOfficeAI/SalomoneUI/wiki/ACP-Setup) — 把終端 AI Agent 整合進來
- [🔌 MCP 工具配置](https://github.com/iOfficeAI/SalomoneUI/wiki/MCP-Configuration-Guide) — 模型上下文協定伺服器設定
- [🌐 WebUI 配置指南](https://github.com/iOfficeAI/SalomoneUI/wiki/WebUI-Configuration-Guide) — WebUI 完整設定教學

**🎯 使用場景**

- [📁 檔案管理](https://github.com/iOfficeAI/SalomoneUI/wiki/file-management) — 讓 AI 幫您整理檔案
- [📊 Excel 處理](https://github.com/iOfficeAI/SalomoneUI/wiki/excel-processing) — AI 驅動的資料處理
- [🎨 圖像生成](https://github.com/iOfficeAI/SalomoneUI/wiki/SalomoneUI-Image-Generation-Tool-Model-Configuration-Guide) — AI 圖像生成
- [📚 更多使用場景](https://github.com/iOfficeAI/SalomoneUI/wiki/Use-Cases-Overview)

**❓ 支援與幫助**

- [❓ FAQ](https://github.com/iOfficeAI/SalomoneUI/wiki/FAQ) — 常見問題和解決方案
- [🔧 配置與使用教學](https://github.com/iOfficeAI/SalomoneUI/wiki/Configuration-Guides) — 完整配置文件
...
- [報告問題](https://github.com/iOfficeAI/SalomoneUI/issues) — 遇到 bug 或有新功能想法？告訴我們
- [發布更新](https://github.com/iOfficeAI/SalomoneUI/releases) — 取得最新版本
...
<td>
  <a href="https://packycode.com" target="_blank">PackyCode</a> 是一家可靠高效的 API 中繼服務提供商，為 Claude Code、Codex、Gemini 等平台提供中繼服務。感謝 PackyCode 為支持 SalomoneUI 用戶低成本使用，為我們的用戶提供專屬 <a href="https://www.packyapi.com/register?aff=salomoneui" target="_blank">9折優惠</a>，點擊此連結並在付款時使用優惠碼 <code>salomoneui</code> 可立減 10%。
</td>
</tr>
</table>
...
[報告 Bug](https://github.com/iOfficeAI/SalomoneUI/issues) · [請求功能](https://github.com/iOfficeAI/SalomoneUI/issues)
