...
> **摘要**: 分析 OpenClaw acpx 与 Zed 编辑器的 ACP 架构设计，提取值得借鉴的模式，对比 SalomoneUI 场景差异
...
- [7. SalomoneUI 与 acpx 的场景差异](#7-salomoneui-与-acpx-的场景差异)
...
SalomoneUI 的 ACP 重构选择 acpx 作为参考实现，原因有三：
...
└── codex-compat.ts            # Codex 兼容层（Codex 特有的协议适配，SalomoneUI 已有自己的 Codex 适配逻辑，不借鉴）
```

### 2.2 核心设计特点

acpx 的架构有四个突出特点，每一个都对应 SalomoneUI 当前实现的一个弱点：

| acpx 做法                                                 | SalomoneUI 当前做法                     | 差距               |
| --------------------------------------------------------- | ----------------------------------- | ------------------ |
...
关键观察：独立工具模块与 AcpClient 是松耦合的（虚线），它们是纯函数、无状态、无外部依赖，这使得它们可以直接移植到 SalomoneUI。
...
Zed 的 ACP 实现用 Rust 所有权模型实现了确定性的进程生命周期管理，与 SalomoneUI 当前做法形成鲜明对比：

| Zed 做法                                                              | SalomoneUI 当前做法                                 | 差距               |
| --------------------------------------------------------------------- | ----------------------------------------------- | ------------------ |
...
**启示**：SalomoneUI 应引入同一个 SDK，消除 `AcpConnection` 中 1,100+ 行的手搓代码。SDK 处理的是协议层的机械性工作（消息序列化/反序列化、请求/响应匹配、notification 路由），这些工作没有业务价值，且手搓容易出错。
...
**对比 SalomoneUI**：当前实现收到 error response 时只做 `message.error?.message || 'Unknown ACP error'`，丢失了 code 和 data，然后用字符串 `includes()` 匹配来分类。信息在第一步就丢了，后续无论怎么改善分类逻辑都无济于事。
...
SalomoneUI 的 `killChild` 直接从 SIGTERM 开始，缺少 stdin 关闭这个最优雅的阶段。stdin.end() 给了 agent 进程自行清理（保存状态、关闭连接等）的机会，这对会话恢复的可靠性很重要。
...
SalomoneUI 当前没有等价机制。当需要调试特定 agent 的协议交互时，只能手动在代码中加 console.log。

---

## 5. 可直接复用的模块

以下模块与 acpx 业务逻辑无关，是纯函数或纯数据转换，可直接移植到 SalomoneUI 的新架构中。
...
**替代 SalomoneUI 的**：`utils.ts` 中的 `waitForProcessExit()`（50ms 轮询）、`isProcessAlive()`（signal 探测）、`acpConnectors.ts` 中的 `cliPath.split(' ')`（不支持引号）。
...
校验内容包括：`jsonrpc: "2.0"` 版本号、`id` 类型（`string | number | null`）、`error` 结构体（`{code: number, message: string}`）等。SalomoneUI 当前完全缺失这类校验。
...
这是**最有价值的单个函数**。SalomoneUI 当前收到 error response 时只做 `message.error?.message || 'Unknown ACP error'`，丢失了 code 和 data，导致后续无法按 code 分类错误。`extractAcpError()` 解决了这个信息丢失问题。
...
移植到 SalomoneUI 时，acpx 的 `OutputErrorCode` 需要映射到新架构的 `AcpErrorCode`。主要映射关系：`RUNTIME` -> `AGENT_ERROR`、`TIMEOUT` -> `PROMPT_TIMEOUT`、`NO_SESSION` -> `SESSION_EXPIRED`、`PERMISSION_DENIED` -> `PERMISSION_DENIED`（保持一致）。完整的 `AcpErrorCode` 枚举定义见 [04-type-catalog.md](04-type-catalog.md)。
...
**替代 SalomoneUI 的**：硬编码 `{ code: -32603, message: ... }`，所有错误都报为 Internal error。
...
**替代 SalomoneUI 的**：`setSessionMode` / `setModel` / `setConfigOption` 都是简单 try-catch + `console.warn`，无法区分不同的错误原因。

---

## 6. 设计理念值得参考但不直接复制的模块

以下模块的代码与 acpx 业务逻辑耦合，不能直接复制，但其设计模式值得在 SalomoneUI 新架构中借鉴。
...
SalomoneUI 的 `handleProcessExit` 做了类似 pending request reject 的事情，但 acpx 的实现更干净（单一入口 `runConnectionRequest`，自动注册/清理）。

### 6.2 Agent 生命周期观察

`attachAgentLifecycleObservers()` 统一监听 `exit` / `close` / `stdout.close` 三个事件，且只记录第一次退出信息（`recordAgentExit` 幂等），维护 `AgentExitInfo` 快照。

SalomoneUI 只监听了 `exit` 事件。在某些边缘情况下（如 agent 进程被外部 kill），`exit` 事件可能不触发，但 `close` 事件会。监听多个事件并取第一次是更稳健的做法。
...
## 7. SalomoneUI 与 acpx 的场景差异

acpx 是 CLI 工具，SalomoneUI 是 Electron 桌面应用。两者虽然使用同一协议，但运行环境和产品需求有显著差异，不能盲目照搬。

### 7.1 关键差异对比

| 维度             | acpx (CLI)                   | SalomoneUI (Electron 桌面应用)                           |
| ---------------- | ---------------------------- | ---------------------------------------------------- |
...
### 7.2 SalomoneUI 必须额外解决的问题

以下是 acpx 不需要考虑、但 SalomoneUI 必须处理的问题：
...
| 维度          | acpx                              | SalomoneUI 当前                              | SalomoneUI 新架构（调整后）               |
| ------------- | --------------------------------- | ---------------------------------------- | ------------------------------------- |
...
SalomoneUI 的复杂度部分来自桌面应用的现实（用户环境千差万别、npx cache 经常腐败）。新架构通过内置 bun/bunx 和 Hub 固定安装目录两个设计决策，从根源上减少了对 npx 的依赖，使启动策略大幅简化。
...
| `error-normalization.ts`    | `errors/errorNormalize.ts`       | 移植核心逻辑，调整 error code 枚举适配 SalomoneUI                                                   |
...
| acpx / Zed 做法     | 不采纳原因                                                                                                                                                                           |
| ------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 两级 agent 解析策略 | SalomoneUI 内置 bun + Hub 固定目录，不需要 node_modules 遍历                                                                                                                             |
| 单 client 模型      | acpx/Zed 的单 client 服务单 session 模型不直接适用——SalomoneUI 需要多 session 并行。但我们采纳了单一所有者模式：每个 session 对应一个 `AcpClient` 实例，由 `AcpRuntime` 统一管理生命周期 |
| CLI 交互式权限      | SalomoneUI 使用 GUI 权限卡片，权限流程更复杂（始终允许、超时、多选项）                                                                                                                   |
