...
SalomoneUI 设置页 > Agent > Local Agent 页，APP 启动时自动扫描本地已安装的 CLI backend（Claude Code、Opencode、Kimi 等），将检测到的 agent 展示在列表中供用户使用。
...
3. **不知道有哪些可用**：用户不知道 SalomoneUI 支持哪些 agent backend，缺少发现入口
...
| **远程** | 从 GitHub `salomoneui/hub` 仓库拉取的 index.json 中，存在于远程但不在内置列表中的 extension | 需要网络 |
...
```json
{
  ...
  "extensions": {
    "aionext-auggie": {
      ...
      "version": "1.0.0",
      "description": "Integrates Augment Code as an ACP adapter in SalomoneUI.",
    ...
    "aionext-claude": {
      ...
      "version": "1.0.0",
      "description": "Integrates Anthropic's Claude Code CLI as an ACP adapter in SalomoneUI.",
      "author": "Aionui Official",
      "engines": {
        "salomoneui": "^1.0.0"
      },
...
`engines.salomoneui` 定义了兼容的 SalomoneUI 版本范围，
...
判断 Hub extension 的展示状态:
  ...
  extension 目录不存在 (~/.salomoneui/extensions/aionext-xxx/)
...
| 状态信号           | 数据来源                                         | 说明           |
| ------------------ | ------------------------------------------------ | -------------- |
| extension 是否存在 | `ExtensionLoader` 扫描 `~/.salomoneui/extensions/`   | 已有能力       |
...
APP 启动
  │
  ├── 1. ExtensionLoader 扫描 ~/.salomoneui/extensions/ (现有逻辑)
  │      → 得到已安装 extension 列表
  │
...
用户点击 Install
  ...
  │
  ├── 4. 解压到临时目录
  │      └── ~/.salomoneui/extensions/.tmp/aionext-xxx/
  │
  ├── 5. 校验 aion-extension.json 合法性
  │      └── 失败 → 清理临时目录, 状态 → install_failed
  │
  ├── 6. 原子移动到正式目录
  │      └── .tmp/aionext-xxx/ → ~/.salomoneui/extensions/aionext-xxx/
  │
...
用户点击 Uninstall (更多菜单中)
  ...
  │      └── 失败 → 仅 warn 日志, 不阻塞卸载
  │
  ├── 5. 删除 ~/.salomoneui/extensions/aionext-xxx/ 目录
  │
...
用户点击 Retry (在 install_failed 状态)
  │
  ├── 检查 ~/.salomoneui/extensions/aionext-xxx/ 是否已存在
...
```
SalomoneUI.app/Contents/Resources/
└── hub/
    ├── index.json                   ← 构建 APP 时从 salomoneui/hub 的 dist/ 复制
...
拉取 remote-index.json
  ├── 优先: GitHub Raw
  │   https://raw.githubusercontent.com/salomoneui/hub/main/dist/index.json
  │
  ├── 备选: jsDelivr CDN
  │   https://cdn.jsdelivr.net/gh/salomoneui/hub@main/dist/index.json
  │
  ├── 超时: 5s (不阻塞 APP 启动)
  ├── 重试: 1 次
  │
...
for (const hubAgent of hubAgents) {
  ...
  const extDir = `~/.salomoneui/extensions/${hubAgent.name}/`;
