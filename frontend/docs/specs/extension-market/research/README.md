...
> 目的：深度调研 SalomoneUI Extension 系统现状，为 Agent Hub / Assistant Hub / MCP Hub / Skill Hub 等市场化建设提供基础
...
Extension 系统是 SalomoneUI 的插件化基础设施，允许第三方通过声明式 manifest (`aion-extension.json`) 向 SalomoneUI 贡献多种能力类型。系统横跨 Electron 主进程、渲染进程和 Worker Thread 三层。
...
- Worker Thread 沙箱的消息路由已修复（PR [#1991](https://github.com/iOfficeAI/SalomoneUI/pull/1991)：Worker→Host RPC、事件转发、storage API），但 `createSandbox()` 尚未被实际调用（ChannelPlugin 和 Lifecycle hooks 仍在主进程裸跑，待迁移）。
