...
> \* **bun 与 npm 的 registry 层完全兼容**：bun 使用相同的 npm registry API (`registry.npmjs.org`)、相同的 .tgz 包格式、支持 `.npmrc` 配置和 `BUN_CONFIG_REGISTRY` 镜像切换。SalomoneUI 已内置 bun 运行时，extension 的 `onInstall` 钩子通过 `bun add` 安装 CLI backend。与 npm 的关键差异见 [Q6 补充：bun 运行时注意事项](#bun-运行时注意事项)。
  ...
  "extensions": [
    {
      ...
      "version": "1.2.0",
      "description": "Anthropic Claude Code CLI agent for SalomoneUI",
      "author": "salomoneui",
...
- SalomoneUI 的 extension 不是 JS 包, 发布到 npm 语义不对
...
# 版本定义在每个 extension 的 aion-extension.json 中
{
  ...
  "engines": { "salomoneui": ">=2.0.0" }  ← 兼容性声明
}
```
...
APP 启动 / 定期 (每 24h)
  ...
  │       AND semver.satisfies(appVersion, remote.engines.salomoneui)
  │       → 标记为 "可更新"
  │
  └── UI 展示 Update 按钮
```
...
- 后续可加: 在安装新版前备份旧版到 `~/.salomoneui/extensions/.backup/`, 支持一键回退

**版本号规范**:

- 严格 semver (`major.minor.patch`)
- CI 校验: manifest 中的 `version` 必须大于上一次发布版本 (防止降版本覆盖)
- `engines.salomoneui` 使用 semver range (如 `>=2.0.0`, `^2.1.0`)
...
async function verifyExtension(tgzBuffer: Buffer, bundleJson: string): Promise<boolean> {
  ...
  await verify(bundle, tgzBuffer, {
    ...
    certificateIdentityRegExp: /https:\/\/github\.com\/salomoneui\/hub\/.*/,
  });
...
- **JS 库成熟**: `sigstore` npm 包支持 Node.js ≥18 (SalomoneUI 的 Electron 满足)
...
const trustedIdentities = [
  /https:\/\/github\.com\/salomoneui\/hub\/.*/, // 官方仓库
...
客户端下载 extension
  ...
  │   └── 版本匹配 → 直接使用 (零网络)
  │
  ├── 2. 检查本地缓存 (~/.salomoneui/cache/)
  │   └── 版本匹配 + integrity 验证 → 直接使用
  │
  ├── 3. 从 GitHub 下载 (主源)
  │   ├── 优先: jsDelivr CDN (国内有节点)
  │   │   https://cdn.jsdelivr.net/gh/salomoneui/hub@main/dist/ext-claude-code-1.2.0.tgz
  │   │
  │   ├── 备选: GitHub Raw
  │   │   https://raw.githubusercontent.com/salomoneui/hub/main/dist/ext-claude-code-1.2.0.tgz
  │   │
  │   └── 备选: GitHub Releases (如果迁移到 Releases)
  │       https://github.com/salomoneui/hub/releases/download/v2026.03/ext-claude-code-1.2.0.tgz
  │
  ├── 4. 下载后
  │   ├── 验证 SHA-512 integrity
  │   ├── 验证 Sigstore 签名 (Phase 2)
  │   ├── 缓存到 ~/.salomoneui/cache/
  │   └── 解压到 ~/.salomoneui/extensions/ext-claude-code/
  │
  └── 5. 所有源失败
      └── 显示错误提示, 建议检查网络
```
...
| 缓存        | `~/.salomoneui/cache/<name>-<version>.tgz`, 安装成功后保留 (方便回退) |
...
下载 .tgz 完成
  ...
  │
  ├── 3. 解压到临时目录
  │   └── ~/.salomoneui/extensions/.tmp/ext-claude-code/
  │
  ├── 4. 二次校验
...
SalomoneUI 内置了 bun 运行时，extension 的 `onInstall` 钩子通过 `bun add` 安装 CLI backend（如 `bun add -g @anthropic-ai/claude-code`）。bun 与 npm 共用 registry API，但有以下实际差异需要在实现中处理：
...
| **全局安装路径**           | `bun add -g` 装到 `~/.bun/install/global/`，bin 链接到 `~/.bun/bin/`，不在系统默认 PATH                                        | APP 启动时将内置 bun 的 globalBinDir 加入 PATH 环境变量；或用 `--globalBinDir` 指向 SalomoneUI 管理的目录                |
...
```mermaid
flowchart TD
  subgraph GitHub["GitHub (salomoneui/hub 仓库)"]
    direction TB
    ExtDirs["extensions/\next-claude-code/\next-opencode/\n..."]
  ...
  subgraph App["SalomoneUI APP"]
    direction TB
    Bundled["内置资源\ndist/ 快照\n(打包进 APP)"]
    IndexMerge["Index 合并器\n内置 + 远程"]
    Cache["本地缓存\n~/.salomoneui/cache/"]
    Installer["Extension 安装器\n下载 → 验证 → 解压 → 激活"]
    ExtDir["~/.salomoneui/extensions/\n已安装 extension"]
...
| 现有模块               | 对接方式                                                    |
| ---------------------- | ----------------------------------------------------------- |
| `ExtensionLoader`      | 安装到 `~/.salomoneui/extensions/` 后, 现有扫描逻辑自动发现     |
...
| 步骤 | 内容                                                                                          | 依赖     |
| ---- | --------------------------------------------------------------------------------------------- | -------- |
| 1.1  | 创建 `salomoneui/hub` GitHub 仓库, 迁入官方 extension                                             | —        |
...
| 1.6  | 客户端: ExtensionInstaller — 下载 .tgz → 验证 integrity → 解压 → 安装到 ~/.salomoneui/extensions/ | —        |
