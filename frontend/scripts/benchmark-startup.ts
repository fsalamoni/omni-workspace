 ...
 * parsing the electron-log file for [SalomoneUI:ready] / [SalomoneUI:init] /
 * [SalomoneUI:process] marks, plus `ready-to-show` / `did-finish-load` /
...
type StartupTiming = {
  ...
  wallTotalMs: number;
  // Parsed from [SalomoneUI:ready] marks
  ...
  // Parsed from [SalomoneUI:init] marks
  initTotalMs: number;
  // Parsed from [SalomoneUI:process] marks
...
function getLogFilePath(): string {
  ...
  if (process.platform === 'darwin') {
    candidates.push(
      path.join(os.homedir(), 'Library', 'Logs', 'SalomoneUI-Dev', `${today}.log`),
      path.join(os.homedir(), 'Library', 'Logs', 'SalomoneUI', `${today}.log`)
    );
  } else if (process.platform === 'win32') {
    const appData = process.env.APPDATA ?? path.join(os.homedir(), 'AppData', 'Roaming');
    candidates.push(path.join(appData, 'SalomoneUI', 'logs', `${today}.log`));
  } else {
    candidates.push(path.join(os.homedir(), '.config', 'SalomoneUI', 'logs', `${today}.log`));
  }
...
// Matches: [SalomoneUI:ready] <label> +<ms>ms
// Matches: [SalomoneUI:init]  <label> +<ms>ms
// Matches: [SalomoneUI:process] <label> +<ms>ms
const MARK_REGEX = /\[SalomoneUI:(ready|init|process)\]\s+([^+]+?)\s+\+(\d+)ms/;
...
function parseStartupLog(lines: string[]): ParsedMarks {
  ...
  for (const line of lines) {
    ...
    if (line.includes('[SalomoneUI] Renderer did-finish-load')) marks.logs.rendererDidFinishLoad = true;
    else if (line.includes('[SalomoneUI] Window ready-to-show')) marks.logs.windowReadyToShow = true;
    else if (line.includes('[SalomoneUI] Showing main window')) marks.logs.showingMainWindow = true;
  }
