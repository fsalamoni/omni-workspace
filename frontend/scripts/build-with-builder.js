...
/**
 * Simplified build script for SalomoneUI
...
try {
  ...
  if (process.platform === 'win32' && builderArgs.includes('--win')) {
    ...
    if (!cleaned) {
      const aionRunning = isProcessRunningWindows('SalomoneUI.exe');
      const electronRunning = isProcessRunningWindows('electron.exe');
      if (aionRunning || electronRunning) {
        console.log('⚠️  Detected running SalomoneUI/Electron process. Attempting to close...');
        killWindowsProcesses(['SalomoneUI.exe', 'electron.exe']);
        cleaned = tryRemoveDir(winUnpackedDir);
        if (!cleaned) {
          console.log('⚠️  Directory still locked. Please close any running SalomoneUI/Electron processes and retry.');
        }
      }
    }
  }
  ...
  try {
    ...
    const winExePath = path.join(outDir, 'win-unpacked', 'SalomoneUI.exe');
    ...
    console.log('⚠️  Windows local build failed after SalomoneUI.exe was produced.');
    if (firstError) {
