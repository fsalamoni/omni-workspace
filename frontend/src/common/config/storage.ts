...
export interface IConfigStorageRefer {
  ...
  // Skills Market: whether the salomoneui-skills builtin skill is enabled
 ...
 * 会话来源类型 - 标识会话创建的来源
 */
export type ConversationSource = 'salomoneui' | 'telegram' | 'lark' | 'dingtalk' | 'weixin' | 'wecom' | (string & {});
...
interface IChatConversation<T, Extra> {
  ...
  /** 会话来源，默认为 salomoneui / Conversation source, defaults to salomoneui */
...
export interface IMcpServer {
  ...
  /** Built-in MCP server managed by SalomoneUI (hide edit/delete in UI) */
  builtin?: boolean;
}
