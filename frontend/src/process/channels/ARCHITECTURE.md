...
Channels 是 SalomoneUI 的多平台 AI 助手框架，将 SalomoneUI 的 AI 能力（Gemini、Claude、Codex）通过即时通讯平台暴露给远程用户。目前支持三个平台：
...
1. **ipcBridge** → Renderer 进程（更新 SalomoneUI 桌面界面）
2. **channelEventBus** → ChannelMessageService（更新 IM 平台消息）

这两条路径互不干扰，实现了 SalomoneUI 桌面端和 IM 端的同步显示。
...
4. 本地用户在 SalomoneUI Settings → Channels 中点击 "Approve"
...
-- source 列: 'salomoneui' | 'telegram' | 'lark' | 'dingtalk'
