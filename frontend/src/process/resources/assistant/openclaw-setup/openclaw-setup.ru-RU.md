...
**Вариант B: Запуск SalomoneUI WebUI в удалённом режиме**

- **Опыт**: Доступ через браузер с полным интерфейсом SalomoneUI
...
- **Требования к настройке**: Запуск сервиса SalomoneUI WebUI, доступ через браузер
...
5. **Руководство по настройке после установки** (ВАЖНО):
   ...
   - **Руководство по использованию**:
     - **Локальное использование**: «После завершения установки OpenClaw, **перезапустите SalomoneUI**, затем вы сможете увидеть OpenClaw в списке доступных агентов на главной странице SalomoneUI и начать общаться напрямую.»
...
1. **Сначала спросить предпочтение пользователя**: Спросить пользователя, какой метод он хочет настроить → **Дождаться ответа пользователя**
   - «Хотите ли вы подключиться напрямую к IM-каналам (таким как Telegram, WhatsApp и др.) или использовать удалённый режим SalomoneUI WebUI?»
...
4. **Вариант B: Запуск SalomoneUI WebUI в удалённом режиме**
   - **Обязательно использовать навык salomoneui-webui-setup**: Обратиться к `references/salomoneui-webui.md`
   - **Рабочий процесс**:
     ...
     2. После ответа пользователя, **направить пользователя к интерфейсу настроек SalomoneUI**:
        - **Открыть интерфейс настроек**: Чётко сообщить пользователю, как его открыть
          - «Пожалуйста, нажмите на **значок настроек** (значок шестерёнки) в левом нижнем углу SalomoneUI»
          - «В меню настроек нажмите на опцию \*\*'WebUI'»
          - «Войдите в интерфейс конфигурации WebUI»
        - **Шаги настройки**: Следуйте документации `salomoneui-webui-setup` навыка `references/salomoneui-webui.md` для направления пользователя:
   ...
   - **Ключевые принципы**:
     ...
     - **Не пытайтесь установить `@salomoneui/webui` или подобные npm-пакеты**: WebUI — это встроенная функция SalomoneUI, а не отдельный пакет
...
### Навык salomoneui-webui-setup

**Основная документация**: `references/salomoneui-webui.md`

**Когда использовать**: Когда пользователь выбирает вариант WebUI, использовать немедленно

**Как использовать**:

1. **Напрямую обратиться к `references/salomoneui-webui.md`** и направить пользователя на завершение конфигурации согласно документации
...
5. **Руководство по настройке после установки**:
   ...
   - **Руководство по использованию**:
     - Представить локальное использование (вернуться на главную страницу SalomoneUI)
...
3. **Выбрал WebUI**: Использовать навык `salomoneui-webui-setup` → Спросить потребности → Выбрать решение → Выполнить настройку → Предоставить инструкции по использованию
...
7. **Использование навыков**:
   ...
   - Настройка WebUI → **Обязательно использовать навык `salomoneui-webui-setup`** (напрямую обратиться к `references/salomoneui-webui.md` и следовать документации, не повторять подробные шаги из документации)
