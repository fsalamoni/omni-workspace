# SalomoneUI WebUI 配置指南

## 概述

SalomoneUI 支持 WebUI 模式，允许通过浏览器访问应用。这对于远程使用 OpenClaw 非常有用。SalomoneUI 提供三种远程连接方式，满足不同场景的需求。

**重要**：WebUI 配置应通过 SalomoneUI 设置界面完成，无需使用命令行。本指南将引导你如何在设置界面中完成配置。
...
1. 在 SalomoneUI 主界面，点击左下角的**设置图标**（齿轮图标）
2. 在设置菜单中，点击 **"WebUI"** 选项
3. 进入 WebUI 配置界面

**方式 2：通过快捷键**

- 在 SalomoneUI 主界面，使用快捷键打开设置（具体快捷键请查看 SalomoneUI 帮助文档）
...
1. 在 SalomoneUI 主界面，点击左下角的**设置图标**
2. 点击 **"WebUI"** 选项
...
1. 确保远程设备与 SalomoneUI 电脑在同一 WiFi 网络
...
- 从办公室访问家庭的 SalomoneUI
- 从手机（使用流量）访问家庭的 SalomoneUI
- 需要跨网络访问，但不想配置公网 IP
...
#### Step 1: 在 SalomoneUI 电脑上配置 WebUI

1. **打开 WebUI 设置界面**：
   - 在 SalomoneUI 主界面，点击左下角的**设置图标**
   - 点击 **"WebUI"** 选项
...
#### Step 2: 在 SalomoneUI 电脑上安装并登录 Tailscale
...
1. 在 SalomoneUI 电脑上，打开 Tailscale 应用
...
#### Step 1: 在服务器上安装 SalomoneUI

按照 SalomoneUI 安装指南在服务器上安装 SalomoneUI 应用。

#### Step 2: 通过设置界面配置 WebUI

1. **打开 WebUI 设置界面**：
   - 如果服务器有图形界面，直接打开 SalomoneUI 应用
...
如果需要 SalomoneUI 开机自启，可以配置 systemd 服务。但建议通过 SalomoneUI 设置界面管理 WebUI，而不是通过命令行。
...
按照 SalomoneUI 安装指南在 macOS 服务器上安装 SalomoneUI 应用。
...
```bash
# 允许端口 25808
sudo /usr/libexec/ApplicationFirewall/socketfilterfw --add /Applications/SalomoneUI.app/Contents/MacOS/SalomoneUI
sudo /usr/libexec/ApplicationFirewall/socketfilterfw --unblockapp /Applications/SalomoneUI.app/Contents/MacOS/SalomoneUI
```
...
启动 WebUI 后，可以通过浏览器访问 SalomoneUI，然后：

1. **在首页找到 OpenClaw 入口**（ACP 代理列表）
2. **直接与 OpenClaw 对话**
3. **享受完整的 SalomoneUI 界面功能**：
   - 文件预览和管理
   - 多对话管理
   - 完整的工具和技能支持
...
- [SalomoneUI Wiki - Remote Internet Access Guide](https://github.com/iOfficeAI/SalomoneUI/wiki/Remote-Internet-Access-Guide)
- [SalomoneUI Wiki - WebUI Configuration Guide](https://github.com/iOfficeAI/SalomoneUI/wiki/WebUI-Configuration-Guide)
...
1. **打开设置**：点击 SalomoneUI 左下角的**设置图标** → 点击 **"WebUI"**
