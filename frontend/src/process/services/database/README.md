# SalomoneUI Database System

本文档介绍 SalomoneUI 的新数据库系统，它使用 **better-sqlite3** (主进程) 作为持久化存储。
...
- **数据库文件**: `{userData}/config/salomoneui.db`
- **图片文件**: `{userData}/data/images/`

其中 `{userData}` 为：

- macOS: `~/Library/Application Support/SalomoneUI/`
- Windows: `%APPDATA%/SalomoneUI/`
- Linux: `~/.config/SalomoneUI/`
...
直接复制 `salomoneui.db` 和 `salomoneui.db-wal` 文件即可。

## API 参考

### SalomoneUIDatabase 主要方法
