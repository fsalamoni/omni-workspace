...
/**
 * MCP源类型 - 包括所有ACP后端和SalomoneUI内置
 */
export type McpSource = AcpBackendAll | 'gemini' | 'salomoneui' | 'aionrs';
  ...
  protected async testStdioConnection(
    ...
    try {
      ...
      if (
        ...
        if (isNpx) {
          return {
            success: false,
            error:
              'Bundled bun runtime is unavailable. Please reinstall SalomoneUI or use a direct stdio command instead of npx.',
          };
        }
        return {
          success: false,
      ...
      if (errorCode === 'EACCES' || errorMessage.includes('EACCES') || errorMessage.includes('permission denied')) {
        return {
          success: false,
          error: `Permission denied when running "${transport.command}". Please check file permissions or reinstall SalomoneUI.`,
        };
      }
