...
// Registry file: ~/.salomoneui-cdp-registry.json
// ---------------------------------------------------------------------------

export const DEFAULT_CDP_PORT = 9230;
export const CDP_PORT_RANGE_START = 9230;
export const CDP_PORT_RANGE_END = 9250;
const CDP_REGISTRY_FILE = path.join(os.homedir(), '.salomoneui-cdp-registry.json');
...
function findAvailablePort(preferredPort: number): number {
  ...
  console.log(
    `[CDP] Port ${preferredPort} is occupied by another SalomoneUI instance, scanning range ${CDP_PORT_RANGE_START}-${CDP_PORT_RANGE_END}`
  );
  ...
  console.warn(
    `[CDP] All ports in range ${CDP_PORT_RANGE_START}-${CDP_PORT_RANGE_END} are used by active SalomoneUI instances, trying ${preferredPort}`
  );
  return preferredPort;
}
