...
export const getTempPath = () => {
  ...
  return path.join(rootPath, 'salomoneui');
};
 ...
 * Release builds use ~/.salomoneui; dev builds use ~/.salomoneui-dev.
 ...
 * Release builds use ~/.salomoneui-config; dev builds use ~/.salomoneui-config-dev.
...
export async function verifyDirectoryFiles(dir1: string, dir2: string): Promise<boolean> {
  try {
    ...
    console.warn('[SalomoneUI] Error verifying directory files:', error);
    return false;
  }
}
...
export const copyFilesToDirectory = async (
  ...
  for (const file of files) {
    ...
    try {
      ...
      console.warn(`[SalomoneUI] Source file does not exist, skipping: ${absoluteFilePath}`);
      console.warn(`[SalomoneUI] Original path: ${file}`);
      // 跳过不存在的文件，而不是抛出错误
      continue;
    }
    ...
    try {
      ...
      console.error(`[SalomoneUI] Failed to copy file from ${absoluteFilePath} to ${destPath}:`, error);
      // 继续处理其他文件，而不是完全失败
    }
