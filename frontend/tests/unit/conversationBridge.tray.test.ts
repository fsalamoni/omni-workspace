...
const {
  ...
  return {
    ...
    mockConversationService: {
      createConversation: vi.fn(async () => ({ id: 'conv-created', name: 'Created Conversation', source: 'salomoneui' })),
      deleteConversation: vi.fn(async () => {}),
      updateConversation: vi.fn(async () => {}),
      getConversation: vi.fn(async () => ({ id: 'conv-1', source: 'salomoneui', name: 'Original Name', type: 'gemini' })),
      createWithMigration: vi.fn(async () => ({ id: 'conv-migrated', source: 'salomoneui' })),
    },
    mockWorkerTaskManager: {
      getTask: vi.fn(),
...
describe('conversationBridge tray sync', () => {
  ...
  it('refreshes tray menu after creating a conversation', async () => {
    ...
    expect(result).toEqual({ id: 'conv-created', name: 'Created Conversation', source: 'salomoneui' });
