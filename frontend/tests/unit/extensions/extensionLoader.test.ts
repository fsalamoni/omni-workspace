...
vi.mock('electron', () => ({
  app: {
    isPackaged: false,
    getPath: vi.fn(() => '/tmp/salomoneui-test'),
  },
}));
...
describe('extensions/ExtensionLoader', () => {
  it('prioritizes explicit env extensions over duplicate user-installed extensions and skips implicit examples', async () => {
    const sandbox = createTempDir('salomoneui-loader-');
    ...
    createExtension(path.join(homeDir, '.salomoneui-dev', 'extensions'), 'ext-shadow', 'ext-shadow', '1.0.0');
  ...
  it('keeps E2E discovery hermetic by ignoring user, appdata, and implicit example sources', async () => {
    const sandbox = createTempDir('salomoneui-loader-e2e-');
    const homeDir = path.join(sandbox, 'home');
    const envDir = path.join(sandbox, 'env-extensions');
    const projectRoot = path.join(sandbox, 'project');
    const appDataExtensionsDir = path.join(homeDir, 'AppData', 'Roaming', 'SalomoneUI', 'extensions');
    ...
    createExtension(path.join(homeDir, '.salomoneui-dev', 'extensions'), 'user-only', 'user-only');
